...
def main():
    ...
    window_name = "Night Vision Tracking (CLAHE + Haar)"
    ...
    print("Night Vision Brain Online. Hunting for faces with >5.5 confidence...")
    ...
    while True:
        ...
        # This massively improves contrast in washed-out IR night vision cameras
        ...
        # Filter faces by confidence > 5.5
        valid_faces = []
        if len(faces) > 0:
        ...
        if len(valid_faces) > 0:
            ...
            # Find the largest face by area among the valid ones
            largest_face_tuple = max(valid_faces, key=lambda data: data[0][2] * data[0][3])
            
            largest_face, confidence = largest_face_tuple
            fx, fy, fw, fh = largest_face
            
            print(f"[NIGHT-HAAR] Face Detected at ({fx}, {fy}) | Confidence (Weight): {confidence:.2f}")
            
            # Draw bounding box around the detected face
            cv2.rectangle(frame, (fx, fy), (fx+fw, fy+fh), (255, 0, 0), 2)
            cv2.putText(frame, f"NIGHT FACE (Conf: {confidence:.2f})", (fx, max(20, fy - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            ...
            # Fallback to Person Detection if no face is found (or all faces are <= 5.5 confidence)
